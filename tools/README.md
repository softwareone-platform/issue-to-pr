...
Pre-publish gates for the plugins in this repository. Standard library only, no `pip install`.
...
|---|---|
| `json` | A manifest, step registry or eval set that does not parse. A malformed step registry silently resurrects a deleted step. |
