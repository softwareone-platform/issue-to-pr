...
> **How to read a factual claim in here.** A paraphrase in this file gets trusted as ground truth and has already been used to design a test fixture — where it turned out to be wrong. So: anything stated about repo contents or script behaviour was **re-verified against the files on 2026-08-10** and carries a `file:line` where it is specific. Four claims in here **cannot** be checked from this repo and are marked *(external)* where they appear — the description-length cap and its setting name, the no-nested-subagent constraint, and the plugin `hooks/` path (no plugin ships one, so it has never been exercised). Treat an *(external)* claim as a working assumption, and if you are about to build on one, verify it against Claude Code's own docs first. Anything else you find unmarked and un-cited is stale by default — check it, then add the citation.
...
- Crucial constraint: **a subagent cannot spawn another subagent** *(external — runtime behaviour; the whole orchestrator/writer/verifier split rests on it, so confirm it against Claude Code's docs before designing around it)*. So writers/verifiers never spawn follow-ups themselves — they *emit structured output* and the orchestrator merges it and does any downstream spawning.
