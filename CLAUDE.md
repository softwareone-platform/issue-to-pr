...
3. **The `test-authoring` prompts do not match this repo** — the five eval files under `plugins/test-authoring/skills/*/evals/` name a C#/.NET billing domain (`BillingService.CreateInvoice`, `POST /invoices`, `InvoiceConsumer`). Run here, the model greps, finds nothing, and correctly declines — so every positive is a false miss. Running them needs a fixture repo in that domain, or prompts rewritten to this repo's own subject matter. **This does not apply to `review-plan-risk`'s eval set** (the sixth and only other one): 0 of its 12 entries mention that domain, so it is blocked by 1 and 2 alone.
