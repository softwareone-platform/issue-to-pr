...
The recommendation is **advisory, not a hard gate.** Standalone, it tells the human where the diagnosis stands; inside the orchestrator, it is the artifact the next phase reads — and the human is asked at this step too on a HALT or RESOLVE verdict, then decides again at the plan-approval gate, so this step is a place a run can stop rather than only a note for a later one. State every degradation prominently where it applies — `Jira anchor not available`, `seam with <repo> not assessed`, `static-only`, `not-verified` — so reduced assurance is always visible, never silently assumed.
