...
Disclosed reference for `open-pr` (Step 3, default description shape, item 4). Read only when you decide to include a diagram in a PR description — it is always optional, and it is left out entirely when the layout learned for the target is terser than the default (a bare cross-reference to an originating PR, say). The inline item keeps the decision (when to include, and the core constraints); this file holds the drawing mechanics, the rationale, and an illustrative shape.
