...
**Re-check the edited description against the platform's own limit before creating.** An edit lands after every check the draft passed, and Azure DevOps **rejects** a description over 4,000 characters rather than truncating it — so an over-long edit fails the create after the human has already approved. Where the backend adapter names a limit, measure against it and say so before creating; where it names none, there is nothing to check. Then:
...
- **The publish is rejected** — `git push -u origin <branch>` fails because the branch is already on the remote and has diverged, typically after a rebase or an amend. **Do not create the PR, and do not force-push.** Report the divergence in both directions (`git rev-list --left-right --count origin/<branch>...HEAD`) and hand it back: integrating the remote commits and re-running is one remedy, a force-push is the other, and the safety rails allow that one only on an explicit request. Say plainly that this is a **push failure, not a declined confirmation** — they leave the same trace (no branch, no PR) and a caller that cannot tell them apart records the wrong reason for the stop.
