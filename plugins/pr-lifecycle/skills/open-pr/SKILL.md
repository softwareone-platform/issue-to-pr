...
description: >
  ...
  with a ticket link and a body shaped the way those PRs are shaped.
...
**Expect the answer to be "no structure", and treat that as a finding rather than a failure.** A short, plain, unformatted description is what most people write most of the time; headings, bullet lists, and diagrams are the exception. So a sample of one-line bodies means the convention *is* a one-line body — draft one. Do not read a plain sample as a gap to improve on, and do not promote it to bullets because bullets look more thorough: that is imposing a shape, which is the one thing this step exists to avoid. The same applies to whatever a layout points at — where a majority of the sample references the pull request a change was ported from, reproduce that; where only some do, it is not the convention.
