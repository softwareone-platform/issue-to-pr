...
Open a pull request (Azure DevOps or GitHub) for the current branch, with a title and description that follow the calling user's own past-PR conventions (learned at runtime) instead of a fixed house style. The draft is always presented for confirmation, and the PR is never created without explicit approval.
...
- Always English; no git `Co-Authored-By` trailer (that is a commit rule). AI-provenance markers are **off by default**: neither the `🤖` footer nor the `ai-assisted` label is added unless the invoking request explicitly asks to mark the PR as AI-assisted. When opted in, the footer is the last line and the label is best-effort (see SKILL.md and the backend adapter). *(Changed: earlier versions always added both. Ask for them explicitly if you want them — e.g. an audit/disclosure workflow.)*
