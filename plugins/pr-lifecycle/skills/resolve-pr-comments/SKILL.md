...
1. **Commit** the kept fixes (generic git), following the repo's own commit convention. Prefix with the branch's ticket id **if one is clearly present** — a Jira key (`ABC-123`), or on a GitHub remote an issue number (`#<n>`), the same forms the tracker adapters define — as `<ticket> <summary>` (≤50 characters, no `Co-Authored-By`); otherwise use a plain `<summary>`. (Squash-merge repos collapse per-commit messages anyway, so do not over-invest here.) Commit only the changes made for these comments. One commit for the batch is the default.
