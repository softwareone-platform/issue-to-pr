...
- Fix commits — follow the repo's own convention, **read off its own history** (`git log --format=%s` for the subject shape and length) rather than assumed. A ticket prefix is used only if the history uses one, in the form the history uses. Where there is no history to learn from, the fallback is a plain summary, ≤50 chars. Only the changes made for the comments are committed.
