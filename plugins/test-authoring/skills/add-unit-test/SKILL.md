...
Where the writer returned a **seed proposal** instead of files, spawn the verifier for it too, substituting `seed_proposal: true` plus the writer's `proposed_path`, `proposed_content` and `invented_choices` for the `files_created` / `sibling_tests_referenced` / `test_results` / `build_status` lines of the template below. The verifier's applicable check set for that case is defined in `common-verifier-checks.md` → "Reviewing a seed proposal"; you do not need to read it to fill the prompt, only to know the substitution is expected.
