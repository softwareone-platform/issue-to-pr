...
| **Claimed actions happened** | Every reported `updated` method actually differs from `git show HEAD` (whitespace-only does not count, evidence attributed per method not per file); every reported `deleted` method is actually gone; every method reported but **not** planned is a violation against the report (E3 forbade touching it); and every planned action with no visible change is a violation unless a named exemption applies — a skipped file, an E1 stop, a carried-forward fix round, or out of scope. A baseline that cannot be obtained is a violation too, never a pass. The one positive check — the other five ask whether something was done wrongly, this one asks whether anything was done |
