...
| **Claimed actions happened** | The one positive check, narrowed twice, and now resting on a single kind of evidence: **presence**. A method reported `deleted` that is **still in the file** is a violation, including under a new name — a rename is not a deletion. A method reported `updated` that is **absent** is a violation — a deletion wearing an update's label. And a `changes_applied` row naming a **file the action record does not name** is a violation, since E3 forbade touching it. Nothing here reads a diff or a baseline, so all three hold on dirty, untracked, and consent-proceeded files alike; there is no degraded mode, no exemption, and no precedence rule. Everything else is a report, not a finding — a planned action nobody reported, a reported action the record planned differently, an `action` value outside the two allowed. The cost is stated rather than hidden: it checks existence and never content (a method reported `updated` and left untouched passes), it reads only the record's files, it cannot see a run whose audit planned nothing, and it cannot see a writer that claimed nothing |
