...
| **Claimed actions happened** | The one positive check, and deliberately narrow: only two verdicts, both read straight off disk. A reported `deleted` method that is **still in the file** is a violation (presence needs no baseline, so this holds even on a dirty file), and a reported `updated` method whose file is **byte-identical to `HEAD`** is a violation **when that file was confirmed tracked and clean**. No exemptions and no precedence rule. Everything else it sees is a report, not a finding — a method reported but not planned, a planned action nobody reported, an `updated` row whose baseline is unusable. The cost is stated rather than hidden: it cannot tell whether the *right* thing changed, cannot see a run whose audit planned nothing, and cannot see a writer that claimed nothing |
