...
This doc describes the **per-type** `verify-update-<type>-test-agent` pattern — one agent per supported code-driven test type (`test-authoring:verify-update-unit-test-agent`, `test-authoring:verify-update-integration-test-agent`, and `test-authoring:verify-update-component-test-agent` when component update lands). The agents share the same five-step flow (below); type-specific extensions live in each per-type file — unit writers focus on deletion confirmations and build/run, integration adds `env_failure` distinction.
