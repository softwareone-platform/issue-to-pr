...
```mermaid
flowchart TD
    T(["/disconfirm-first:review-issue-fact<br>or trigger phrase"]) --> G{"An issue to fact-check,<br>not a plan / fix / tracker edit?"}
    G -- "no — scope guard" --> STOP(["Stop: wrong skill<br>(review-plan-risk / review-code-risk)"])
    G -- "yes" --> S1["Step 1 — resolve the issue (text, Jira, or GitHub);<br>codebase is the oracle;<br>cross-repo: read on-disk siblings,<br>else needs-info / seam"]
