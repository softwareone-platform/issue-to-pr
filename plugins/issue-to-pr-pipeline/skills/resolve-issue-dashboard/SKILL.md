...
- **Pipeline** — the twelve steps (`a-fact-check` … `done`) as nodes; status is derived from `state.md`'s `next-step` (earlier steps completed, the current one running or paused, later ones pending).
