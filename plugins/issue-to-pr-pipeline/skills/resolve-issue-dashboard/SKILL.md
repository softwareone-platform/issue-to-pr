...
- **Stat cards** — pipeline step `N` of the registry's total, elapsed (wall-clock) duration, tool-call count, and tokens in / cached / out. The three token figures are the API's own, each response counted **once**: `in` is the cache-**miss** input only, `cached` is cache reads plus cache writes, `out` is model output. Two things follow. One response is written to the transcript as several records that all repeat the same `usage`, so any per-record sum inflates every figure — the dashboard de-duplicates on the message id, which is why these numbers can read lower than a hand count of the same file. And the three are deliberately not blended: a cached read costs a fraction of a fresh one, so one combined "input" number would read as spend it is not. Treat all three as **volume, not price**.
