...
- **Pipeline** — the registered steps (`a-fact-check` … `done`) as nodes; status is derived from `state.md`'s `next-step` (earlier steps completed, the current one running or paused, later ones pending). Each completed node also shows its wall-clock beneath the label, and a step re-entered by a gate revise shows a `×N` count — both read from the run's append-only `timings.md`, so you can see which step ate the run's time. A completed node's tooltip additionally shows its **output-token total** — a compute-vs-waiting signal: a slow step with few output tokens was mostly waiting (a gate, a pause), not computing.
...
- **Stat cards** — pipeline step `N` of the registry's total, elapsed (wall-clock) duration, tool-call count, and tokens in / cached / out. The three token figures are the API's own, each response counted **once**: `in` is the cache-**miss** input only, `cached` is cache reads plus cache writes, `out` is model output. Two things follow. One response is written to the transcript as several records that all repeat the same `usage`, so any per-record sum inflates every figure — the dashboard de-duplicates on the **pair** of source file and message id, which is why these numbers can read lower than a hand count of the same file. The pair matters: message ids are not unique across transcript files, so keying on the id alone would silently discard a subagent's response whenever it collided with a main-session one. And the three are deliberately not blended: a cached read costs a fraction of a fresh one, so one combined "input" number would read as spend it is not. Treat all three as **volume, not price**.
...
- **Test-contention banner** — a global warning appears when two or more runs (any repo) are positioned at a test-executing step (the ones that run integration tests). Those steps usually need shared local infrastructure — containers, databases, emulators — so running them at once can conflict, race, or starve. The banner names the colliding runs so you can stagger them; it only warns, since the dashboard never controls a run.
...
- **Which session a run is read from** — the most recently active session in that repo that has referenced the run's handoff dir (`.claude/resolve/<ticket>/`) anywhere in its transcript, among the sessions active since the run's `started`; so resuming `resolve-issue` in a fresh session keeps the view live, and a session that only mentioned the ticket, or went quiet before the run began, cannot claim it. With no such session the newest one active since `started` is used, and with none at all nothing is tailed.
...
- **Pipeline definition** — the step ids, labels, components, and order come from the shared registry `issue-to-pr-pipeline/resources/resolve-issue-steps.json` (loaded at startup; an embedded copy is the fallback if it cannot be read). Add, rename, or reorder a step there and the dashboard picks it up — no code change, but **restart the server**: the registry is read once at import, so a running dashboard keeps serving the snapshot it started with and a browser reload will not refresh it. A `next-step` value absent from the registry still degrades the pipeline to idle / all-pending rather than guessing a position.
...
- **Metrics are run-scoped** — one session is not one run: compacting keeps every earlier record in the transcript and a resume appends to it, so a long-lived session can carry weeks of other tickets ahead of this one. The activity stream, the tool-call count, the tokens and the waiting-for-you signal therefore count only records stamped inside the run — from its `started` to the end of its `ended` second, since a finished run's session often goes on to other work — and skip subagents that went quiet before it began. A run that predates the `started` field falls back to the whole session, so its figures read cleanest in a session dedicated to it. **Elapsed duration is run-scoped too**: it reads the `started` / `ended` UTC timestamps `resolve-issue` records in `state.md`, so it stays correct even when you reopen a finished run later from a newer session — it ticks every second while the run is live (a heartbeat that keeps moving even while paused at a gate) and freezes at `ended` once done. It is **wall-clock, not active time** — a run paused at a gate or resumed the next day includes that idle span, so read it as calendar elapsed, not compute. A run that predates these timestamps (no `started` field) falls back to the session-derived anchors (first event / `state.md` mtime), which can read `-` when viewed from a different session — the only residual session-scoping for duration.
