...
- **Test-contention banner** — a global warning when two or more runs (across all repos) sit on a test-executing step at once; those steps share the host's Podman / SQL / Azurite, so running them together can conflict. It names the colliding runs to stagger and is read-only, like the rest of the dashboard. Which steps count is marked with `runsTests` in the shared registry, so it stays in sync with the pipeline.
