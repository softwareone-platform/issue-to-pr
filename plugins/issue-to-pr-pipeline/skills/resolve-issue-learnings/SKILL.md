...
2. **Detect whether Y is *possible* — structurally, not by name.** Y edits the plugin's own source, which only exists when this skill is being run from a checkout of the plugin's source repo. Confirm both: `git rev-parse --show-toplevel` succeeds, and the resolve-issue source `<root>/plugins/issue-to-pr-pipeline/skills/resolve-issue/SKILL.md` exists and is git-tracked (`git ls-files --error-unmatch <path>` succeeds). This is independent of the folder name and the remote repo name, and works for forks. If any check fails, Y is not possible — run X only.
