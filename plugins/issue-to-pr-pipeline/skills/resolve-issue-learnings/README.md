...
- **Mode by capability, not name.** Y is offered only when the resolve-issue source SKILL.md is git-tracked in the working tree — independent of folder/repo name, fork-friendly. A mis-detection only *offers* Y; it never edits source unprompted.
